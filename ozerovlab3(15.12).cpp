#include <iostream>
#include <fstream>
#include <vector>
#include <string>
#include <algorithm>
#include <unordered_map>
#include <unordered_set>
#include <queue>

using namespace std;

// ==================== Базовые функции ввода ====================
int get_int() {
    int x;
    while (!(cin >> x) || x < 0) {
        cin.clear();
        cin.ignore(1000, '\n');
        cout << "Ошибка! Введите целое число >= 0: ";
    }
    return x;
}

double get_double() {
    double x;
    while (!(cin >> x) || x < 0) {
        cin.clear();
        cin.ignore(1000, '\n');
        cout << "Ошибка! Введите число >= 0: ";
    }
    return x;
}

// ==================== Класс Труба ====================
class Pipe {
public:
    int id;
    string name;
    double length;
    int diameter;
    bool in_repair;
    bool is_connected;  // новая переменная: подключена ли труба к сети
    
    static int next_id;
    
    Pipe() {
        id = next_id++;
        in_repair = false;
        is_connected = false;
    }
    
    void input() {
        cout << "Название трубы: ";
        cin >> ws;
        getline(cin, name);
        cout << "Длина: ";
        length = get_double();
        
        // Ограничение диаметров
        vector<int> allowed_diameters = {500, 700, 1000, 1400};
        bool valid_diameter = false;
        
        while (!valid_diameter) {
            cout << "Диаметр (500, 700, 1000 или 1400 мм): ";
            diameter = get_int();
            if (find(allowed_diameters.begin(), allowed_diameters.end(), diameter) != allowed_diameters.end()) {
                valid_diameter = true;
            } else {
                cout << "Неверный диаметр! Разрешенные значения: 500, 700, 1000, 1400 мм" << endl;
            }
        }
    }
    
    void show() {
        cout << "Труба ID:" << id << " \"" << name << "\" Д:" << diameter 
             << "мм L:" << length << " Ремонт:" << (in_repair ? "Да" : "Нет")
             << " Подкл:" << (is_connected ? "Да" : "Нет") << endl;
    }
    
    void edit() {
        in_repair = !in_repair;
        cout << "Статус ремонта изменен" << endl;
    }
    
    bool match_name(const string& filter) {
        return name.find(filter) != string::npos;
    }
    
    bool match_repair(bool status) {
        return in_repair == status;
    }
    
    bool is_available() {
        return !in_repair && !is_connected;
    }
};

int Pipe::next_id = 1;

// ==================== Класс КС ====================
class KS {
public:
    int id;
    string name;
    int total_shops;
    int working_shops;
    
    static int next_id;
    
    KS() {
        id = next_id++;
    }
    
    void input() {
        cout << "Название КС: ";
        cin >> ws;
        getline(cin, name);
        cout << "Всего цехов: ";
        total_shops = get_int();
        cout << "Работает цехов: ";
        working_shops = get_int();
        while (working_shops > total_shops) {
            cout << "Не может работать больше чем всего! Введите снова: ";
            working_shops = get_int();
        }
    }
    
    void show() {
        double percent = total_shops > 0 ? (100.0 * (total_shops - working_shops) / total_shops) : 0;
        cout << "КС ID:" << id << " \"" << name << "\" Цехов:" << working_shops 
             << "/" << total_shops << " Простой:" << percent << "%" << endl;
    }
    
    bool match_name(const string& filter) {
        return name.find(filter) != string::npos;
    }
    
    bool match_unused(double min_percent) {
        if (total_shops == 0) return false;
        double percent = 100.0 * (total_shops - working_shops) / total_shops;
        return percent >= min_percent;
    }
    
    void start_shop() {
        if (working_shops < total_shops) {
            working_shops++;
            cout << "Запущен цех. Работает: " << working_shops << endl;
        } else {
            cout << "Все цехи уже работают!" << endl;
        }
    }
    
    void stop_shop() {
        if (working_shops > 0) {
            working_shops--;
            cout << "Остановлен цех. Работает: " << working_shops << endl;
        } else {
            cout << "Нет работающих цехов!" << endl;
        }
    }
};

int KS::next_id = 1;

// ==================== Класс для соединения КС трубой ====================
class Connection {
public:
    int pipe_id;
    int ks_in_id;   // КС на входе
    int ks_out_id;  // КС на выходе
    
    Connection(int pipe, int in, int out) : pipe_id(pipe), ks_in_id(in), ks_out_id(out) {}
};

// ==================== Класс Газотранспортная сеть ====================
class GasNetwork {
private:
    vector<Connection> connections;
    unordered_map<int, vector<int>> adjacency_list;  // список смежности
    unordered_map<int, int> in_degree;              // полустепень захода
    
public:
    // Добавить соединение
    void add_connection(int pipe_id, int ks_in_id, int ks_out_id) {
        connections.push_back(Connection(pipe_id, ks_in_id, ks_out_id));
        
        // Обновляем граф
        adjacency_list[ks_in_id].push_back(ks_out_id);
        in_degree[ks_out_id]++;
        if (in_degree.find(ks_in_id) == in_degree.end()) {
            in_degree[ks_in_id] = 0;
        }
    }
    
    // Удалить все соединения с указанной трубой
    void remove_connections_with_pipe(int pipe_id) {
        auto it = connections.begin();
        while (it != connections.end()) {
            if (it->pipe_id == pipe_id) {
                // Удаляем из графа
                int ks_in = it->ks_in_id;
                int ks_out = it->ks_out_id;
                
                // Удаляем из списка смежности
                auto& neighbors = adjacency_list[ks_in];
                neighbors.erase(remove(neighbors.begin(), neighbors.end(), ks_out), neighbors.end());
                
                // Уменьшаем полустепень захода
                in_degree[ks_out]--;
                
                it = connections.erase(it);
            } else {
                ++it;
            }
        }
    }
    
    // Проверка, свободна ли труба
    bool is_pipe_used(int pipe_id) {
        for (const auto& conn : connections) {
            if (conn.pipe_id == pipe_id) {
                return true;
            }
        }
        return false;
    }
    
    // Показать все соединения
    void show_connections() {
        if (connections.empty()) {
            cout << "Нет соединений в сети" << endl;
            return;
        }
        
        cout << "\n=== ГАЗОТРАНСПОРТНАЯ СЕТЬ ===" << endl;
        for (const auto& conn : connections) {
            cout << "КС" << conn.ks_in_id << " -> КС" << conn.ks_out_id 
                 << " (Труба ID:" << conn.pipe_id << ")" << endl;
        }
        cout << "=============================" << endl;
    }
    
    // Топологическая сортировка (алгоритм Кана)
    vector<int> topological_sort() {
        vector<int> result;
        
        // Создаем копию in_degree
        unordered_map<int, int> temp_in_degree = in_degree;
        queue<int> q;
        
        // Находим вершины с нулевой полустепенью захода
        for (const auto& node : temp_in_degree) {
            if (node.second == 0) {
                q.push(node.first);
            }
        }
        
        while (!q.empty()) {
            int current = q.front();
            q.pop();
            result.push_back(current);
            
            // Уменьшаем полустепень захода у соседей
            for (int neighbor : adjacency_list[current]) {
                temp_in_degree[neighbor]--;
                if (temp_in_degree[neighbor] == 0) {
                    q.push(neighbor);
                }
            }
        }
        
        // Проверка на наличие циклов
        if (result.size() != temp_in_degree.size()) {
            cout << "Внимание! В графе обнаружены циклы. "
                 << "Топологическая сортировка невозможна для циклического графа." << endl;
            return vector<int>();
        }
        
        return result;
    }
    
    // Получить все соединения
    const vector<Connection>& get_connections() const {
        return connections;
    }
};

// ==================== Глобальные данные ====================
vector<Pipe> pipes;
vector<KS> ks_list;
GasNetwork network;
ofstream log_file("log.txt", ios::app);

// ==================== Функции логирования ====================
void log(const string& msg) {
    log_file << msg << endl;
}

// ==================== Вспомогательные функции ====================
Pipe* find_pipe_by_id(int id) {
    for (auto& pipe : pipes) {
        if (pipe.id == id) {
            return &pipe;
        }
    }
    return nullptr;
}

KS* find_ks_by_id(int id) {
    for (auto& ks : ks_list) {
        if (ks.id == id) {
            return &ks;
        }
    }
    return nullptr;
}

// ==================== Функции работы с трубами ====================
void add_pipe() {
    Pipe p;
    p.input();
    pipes.push_back(p);
    log("Добавлена труба ID:" + to_string(p.id));
    cout << "Труба добавлена (ID:" << p.id << ")" << endl;
}

void show_pipes() {
    if (pipes.empty()) {
        cout << "Нет труб" << endl;
        return;
    }
    for (auto& p : pipes) p.show();
}

vector<int> find_pipes_by_name() {
    string filter;
    cout << "Поиск по названию: ";
    cin >> ws;
    getline(cin, filter);
    
    vector<int> result;
    for (auto& p : pipes) {
        if (p.match_name(filter)) {
            result.push_back(p.id);
        }
    }
    log("Поиск труб по названию \"" + filter + "\" найдено: " + to_string(result.size()));
    return result;
}

vector<int> find_pipes_by_repair() {
    cout << "Искать в ремонте? (1-да, 0-нет): ";
    int choice;
    cin >> choice;
    bool status = (choice == 1);
    
    vector<int> result;
    for (auto& p : pipes) {
        if (p.match_repair(status)) {
            result.push_back(p.id);
        }
    }
    log("Поиск труб по ремонту найдено: " + to_string(result.size()));
    return result;
}

void batch_edit_pipes(const vector<int>& ids) {
    if (ids.empty()) {
        cout << "Не найдено труб для редактирования" << endl;
        return;
    }
    
    cout << "Найдено " << ids.size() << " труб" << endl;
    cout << "1. Редактировать все\n2. Выбрать конкретные\nВыберите: ";
    int choice = get_int();
    
    vector<int> to_edit;
    
    if (choice == 1) {
        to_edit = ids;
    } else {
        cout << "Введите ID через пробел (0 для завершения): ";
        int id;
        while (cin >> id && id != 0) {
            if (find(ids.begin(), ids.end(), id) != ids.end()) {
                to_edit.push_back(id);
            } else {
                cout << "ID " << id << " не входит в найденные!" << endl;
            }
        }
        cin.clear();
        cin.ignore(1000, '\n');
    }
    
    cout << "1. Изменить статус ремонта\n2. Удалить\nВыберите: ";
    choice = get_int();
    
    if (choice == 1) {
        for (int id : to_edit) {
            Pipe* pipe = find_pipe_by_id(id);
            if (pipe) {
                pipe->edit();
                log("Изменен статус трубы ID:" + to_string(id));
            }
        }
        cout << "Готово!" << endl;
    } else {
        // Удаление
        auto it = remove_if(pipes.begin(), pipes.end(), 
            [&](const Pipe& p) {
                if (find(to_edit.begin(), to_edit.end(), p.id) != to_edit.end()) {
                    // Удаляем соединения с этой трубой
                    network.remove_connections_with_pipe(p.id);
                    return true;
                }
                return false;
            });
        pipes.erase(it, pipes.end());
        log("Удалено труб: " + to_string(to_edit.size()));
        cout << "Удалено " << to_edit.size() << " труб" << endl;
    }
}

// ==================== Функции работы с КС ====================
void add_ks() {
    KS k;
    k.input();
    ks_list.push_back(k);
    log("Добавлена КС ID:" + to_string(k.id));
    cout << "КС добавлена (ID:" << k.id << ")" << endl;
}

void show_ks() {
    if (ks_list.empty()) {
        cout << "Нет КС" << endl;
        return;
    }
    for (auto& k : ks_list) k.show();
}

vector<int> find_ks_by_name() {
    string filter;
    cout << "Поиск по названию: ";
    cin >> ws;
    getline(cin, filter);
    
    vector<int> result;
    for (auto& k : ks_list) {
        if (k.match_name(filter)) {
            result.push_back(k.id);
        }
    }
    log("Поиск КС по названию \"" + filter + "\" найдено: " + to_string(result.size()));
    return result;
}

vector<int> find_ks_by_unused() {
    cout << "Минимальный процент простоя: ";
    double percent = get_double();
    
    vector<int> result;
    for (auto& k : ks_list) {
        if (k.match_unused(percent)) {
            result.push_back(k.id);
        }
    }
    log("Поиск КС по простою найдено: " + to_string(result.size()));
    return result;
}

// ==================== Функции для работы с сетью ====================
void connect_ks() {
    cout << "\n=== СОЕДИНЕНИЕ КС ТРУБОЙ ===" << endl;
    
    // Показать доступные КС
    cout << "Доступные КС:" << endl;
    show_ks();
    
    // Ввод КС входа и выхода
    cout << "ID КС на входе: ";
    int ks_in_id = get_int();
    cout << "ID КС на выходе: ";
    int ks_out_id = get_int();
    
    // Проверка существования КС
    KS* ks_in = find_ks_by_id(ks_in_id);
    KS* ks_out = find_ks_by_id(ks_out_id);
    
    if (!ks_in || !ks_out) {
        cout << "Ошибка! Одна или обе КС не найдены." << endl;
        return;
    }
    
    if (ks_in_id == ks_out_id) {
        cout << "Ошибка! Нельзя соединить КС саму с собой." << endl;
        return;
    }
    
    // Ввод диаметра трубы
    cout << "Диаметр трубы (500, 700, 1000 или 1400 мм): ";
    int diameter = get_int();
    vector<int> allowed_diameters = {500, 700, 1000, 1400};
    
    if (find(allowed_diameters.begin(), allowed_diameters.end(), diameter) == allowed_diameters.end()) {
        cout << "Неверный диаметр! Разрешенные значения: 500, 700, 1000, 1400 мм" << endl;
        return;
    }
    
    // Поиск свободной трубы нужного диаметра
    Pipe* selected_pipe = nullptr;
    for (auto& pipe : pipes) {
        if (pipe.diameter == diameter && pipe.is_available() && !network.is_pipe_used(pipe.id)) {
            selected_pipe = &pipe;
            break;
        }
    }
    
    // Если труба не найдена, предлагаем создать новую
    if (!selected_pipe) {
        cout << "Нет свободной трубы диаметром " << diameter << " мм" << endl;
        cout << "1. Создать новую трубу\n2. Отмена\nВыберите: ";
        int choice = get_int();
        
        if (choice == 1) {
            add_pipe();
            // Берем последнюю созданную трубу
            selected_pipe = &pipes.back();
            if (selected_pipe->diameter != diameter) {
                cout << "Ошибка! Созданная труба имеет другой диаметр." << endl;
                return;
            }
        } else {
            return;
        }
    }
    
    // Создаем соединение
    network.add_connection(selected_pipe->id, ks_in_id, ks_out_id);
    selected_pipe->is_connected = true;
    
    cout << "Соединение создано: КС" << ks_in_id << " -> КС" << ks_out_id 
         << " (Труба ID:" << selected_pipe->id << ")" << endl;
    log("Создано соединение: КС" + to_string(ks_in_id) + " -> КС" + 
        to_string(ks_out_id) + " (Труба ID:" + to_string(selected_pipe->id) + ")");
}

void disconnect_ks() {
    cout << "\n=== УДАЛЕНИЕ СОЕДИНЕНИЙ ===" << endl;
    
    cout << "ID трубы для отключения: ";
    int pipe_id = get_int();
    
    Pipe* pipe = find_pipe_by_id(pipe_id);
    if (!pipe) {
        cout << "Труба не найдена!" << endl;
        return;
    }
    
    // Удаляем соединения с этой трубой
    network.remove_connections_with_pipe(pipe_id);
    pipe->is_connected = false;
    
    cout << "Соединения с трубой ID:" << pipe_id << " удалены" << endl;
    log("Удалены соединения с трубой ID:" + to_string(pipe_id));
}

void show_network() {
    network.show_connections();
}

void topological_sort_network() {
    cout << "\n=== ТОПОЛОГИЧЕСКАЯ СОРТИРОВКА ===" << endl;
    
    vector<int> sorted = network.topological_sort();
    
    if (sorted.empty()) {
        cout << "Граф пуст или содержит циклы" << endl;
        return;
    }
    
    cout << "Топологический порядок КС:" << endl;
    for (int i = 0; i < sorted.size(); i++) {
        KS* ks = find_ks_by_id(sorted[i]);
        if (ks) {
            cout << i + 1 << ". КС" << ks->id << " \"" << ks->name << "\"" << endl;
        } else {
            cout << i + 1 << ". КС" << sorted[i] << " (не найден в списке)" << endl;
        }
    }
}

// ==================== Функции сохранения/загрузки ====================
void save_all() {
    string filename;
    cout << "Имя файла для сохранения: ";
    cin >> ws;
    getline(cin, filename);
    
    ofstream f(filename);
    
    // Сохраняем трубы
    f << pipes.size() << endl;
    for (auto& p : pipes) {
        f << p.id << " " << p.name << " " << p.length << " " 
          << p.diameter << " " << p.in_repair << " " << p.is_connected << endl;
    }
    
    // Сохраняем КС
    f << ks_list.size() << endl;
    for (auto& k : ks_list) {
        f << k.id << " " << k.name << " " << k.total_shops << " " 
          << k.working_shops << endl;
    }
    
    // Сохраняем соединения
    const auto& connections = network.get_connections();
    f << connections.size() << endl;
    for (const auto& conn : connections) {
        f << conn.pipe_id << " " << conn.ks_in_id << " " << conn.ks_out_id << endl;
    }
    
    f.close();
    log("Сохранено в файл: " + filename);
    cout << "Сохранено!" << endl;
}

void load_all() {
    string filename;
    cout << "Имя файла для загрузки: ";
    cin >> ws;
    getline(cin, filename);
    
    ifstream f(filename);
    if (!f.is_open()) {
        cout << "Файл не найден!" << endl;
        return;
    }
    
    // Загружаем трубы
    int pipe_count;
    f >> pipe_count;
    pipes.clear();
    for (int i = 0; i < pipe_count; i++) {
        Pipe p;
        f >> p.id >> p.name >> p.length >> p.diameter >> p.in_repair >> p.is_connected;
        pipes.push_back(p);
        if (p.id >= Pipe::next_id) Pipe::next_id = p.id + 1;
    }
    
    // Загружаем КС
    int ks_count;
    f >> ks_count;
    ks_list.clear();
    for (int i = 0; i < ks_count; i++) {
        KS k;
        f >> k.id >> k.name >> k.total_shops >> k.working_shops;
        ks_list.push_back(k);
        if (k.id >= KS::next_id) KS::next_id = k.id + 1;
    }
    
    // Загружаем соединения
    int conn_count;
    f >> conn_count;
    
    // Создаем временный объект сети
    GasNetwork temp_network;
    
    for (int i = 0; i < conn_count; i++) {
        int pipe_id, ks_in_id, ks_out_id;
        f >> pipe_id >> ks_in_id >> ks_out_id;
        temp_network.add_connection(pipe_id, ks_in_id, ks_out_id);
    }
    
    // Присваиваем новую сеть
    network = temp_network;
    
    f.close();
    log("Загружено из файла: " + filename);
    cout << "Загружено!" << endl;
}

// ==================== Главное меню ====================
int main() {
    log("=== Программа запущена ===");
    
    while (true) {
        cout << "\n=== МЕНЮ ===" << endl;
        cout << "1. Добавить трубу" << endl;
        cout << "2. Добавить КС" << endl;
        cout << "3. Показать все объекты" << endl;
        cout << "4. Поиск и пакетное редактирование труб" << endl;
        cout << "5. Поиск КС" << endl;
        cout << "6. Управление цехами КС" << endl;
        cout << "7. Соединить КС трубой" << endl;
        cout << "8. Показать газотранспортную сеть" << endl;
        cout << "9. Отключить соединения трубы" << endl;
        cout << "10. Топологическая сортировка сети" << endl;
        cout << "11. Сохранить все" << endl;
        cout << "12. Загрузить все" << endl;
        cout << "0. Выход" << endl;
        cout << "Выбор: ";
        
        int choice = get_int();
        
        switch (choice) {
            case 1: add_pipe(); break;
            case 2: add_ks(); break;
            case 3: 
                cout << "\n=== ТРУБЫ ===" << endl;
                show_pipes(); 
                cout << "\n=== КС ===" << endl;
                show_ks();
                break;
                
            case 4: {
                cout << "1. Поиск по названию\n2. Поиск по ремонту\nВыберите: ";
                int search_type = get_int();
                vector<int> found_ids;
                
                if (search_type == 1) {
                    found_ids = find_pipes_by_name();
                } else {
                    found_ids = find_pipes_by_repair();
                }
                
                if (!found_ids.empty()) {
                    cout << "Выполнить пакетное редактирование? (1-да, 0-нет): ";
                    if (get_int() == 1) {
                        batch_edit_pipes(found_ids);
                    }
                } else {
                    cout << "Трубы не найдены" << endl;
                }
                break;
            }
                
            case 5: {
                cout << "1. Поиск по названию\n2. Поиск по проценту простоя\nВыберите: ";
                int search_type = get_int();
                vector<int> found_ids;
                
                if (search_type == 1) {
                    found_ids = find_ks_by_name();
                } else {
                    found_ids = find_ks_by_unused();
                }
                
                if (!found_ids.empty()) {
                    cout << "Найдено КС:" << endl;
                    for (int id : found_ids) {
                        KS* ks = find_ks_by_id(id);
                        if (ks) ks->show();
                    }
                } else {
                    cout << "КС не найдены" << endl;
                }
                break;
            }
                
            case 6: {
                cout << "ID КС: ";
                int id = get_int();
                KS* ks = find_ks_by_id(id);
                
                if (ks) {
                    cout << "1. Запустить цех\n2. Остановить цех\nВыберите: ";
                    int action = get_int();
                    if (action == 1) {
                        ks->start_shop();
                        log("Запущен цех КС ID:" + to_string(id));
                    } else {
                        ks->stop_shop();
                        log("Остановлен цех КС ID:" + to_string(id));
                    }
                } else {
                    cout << "КС не найдена" << endl;
                }
                break;
            }
                
            case 7: connect_ks(); break;
            case 8: show_network(); break;
            case 9: disconnect_ks(); break;
            case 10: topological_sort_network(); break;
            case 11: save_all(); break;
            case 12: load_all(); break;
                
            case 0: 
                log("=== Программа завершена ===");
                log_file.close();
                return 0;
                
            default: 
                cout << "Неверный выбор!" << endl;
        }
    }
}